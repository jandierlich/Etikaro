...
document.getElementById('fileInput').addEventListener('change', async (e)=>{
  ...
  statusEl.innerHTML = '<div class="status-line">Foto wird geladen …</div>';
...
function openEditForNewScan(ocrText, dataUrl){
  ...
  document.getElementById('ocrRaw').textContent = ocrText || '(kein Text erkannt)';
...
function openEditForId(id){
  ...
  document.getElementById('ocrRaw').textContent = entry.ocrText || '—';
...
document.getElementById('saveEntryBtn').addEventListener('click', async ()=>{
  ...
  if(dupId && !currentEditId){
    ...
    const entry = {
      ...
      ocrText: document.getElementById('ocrRaw').textContent === '—' ? '' : document.getElementById('ocrRaw').textContent,
